...
const bypassAuth = async (c: any, next: any) => {
...
app.get("/api/admin/users", bypassAuth, async (c) => {
  const db = c.env.DB;
...
app.put("/api/admin/users/:userId", bypassAuth, async (c) => {
  const db = c.env.DB;
...
app.post("/api/admin/users/:userId/toggle-active", bypassAuth, async (c) => {
  const db = c.env.DB;
  const userId = c.req.param("userId");
  const now = new Date().toISOString();
...
return c.json({ redirectUrl: "https://google.com" });
...
// Get user credits
app.get("/api/users/me/credits", bypassAuth, async (c) => {
  const db = c.env.DB;
...
app.get("/api/sequences", async (c) => {
  const user = { id: "test-user" }; // fake user
  const db = c.env.DB;
...
// Create sequence
app.post("/api/sequences", bypassAuth, zValidator("json", CreateSequenceSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
// Update sequence
app.put("/api/sequences/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.get("/api/sequences/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Delete sequence
app.delete("/api/sequences/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Duplicate sequence
app.post("/api/sequences/:id/duplicate", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Create email block
app.post("/api/sequences/:id/blocks", bypassAuth, zValidator("json", CreateEmailBlockSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
...
// Update email block
app.put("/api/blocks/:id", bypassAuth, zValidator("json", UpdateEmailBlockSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
// Duplicate block
app.post("/api/blocks/:id/duplicate", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const blockId = c.req.param("id");
  const newBlockId = uuidv4();
...
// Delete block
app.delete("/api/blocks/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const blockId = c.req.param("id");
...
// Create connection
app.post("/api/connections", bypassAuth, zValidator("json", CreateConnectionSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = c.req.valid("json");
...
// Delete connection
app.delete("/api/connections/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.get("/api/user-templates", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.delete("/api/user-templates/:id", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.post("/api/sequences/:id/save-template", bypassAuth, zValidator("json", CreateTemplateSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
...
app.post("/api/ai/generate-content", bypassAuth, zValidator("json", GenerateContentSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = c.req.valid("json");
...
app.post("/api/ai/rewrite-content", bypassAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = await c.req.json();
...
// Export sequence
app.post("/api/sequences/:id/export", bypassAuth, zValidator("json", ExportSequenceSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
